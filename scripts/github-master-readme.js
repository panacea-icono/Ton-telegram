...
function buildMasterReadme() {
  ...
  const reposMd = fs.existsSync(reposMdPath)
    ? fs.readFileSync(reposMdPath, 'utf8')
    : '';
...
async function upsertMasterReadme() {
  ...
  if (put.status === 200 || put.status === 201) {
    console.log(
      '✅ Master README actualizado:',
      put.data && put.data.content && put.data.content.html_url
        ? put.data.content.html_url
        : 'OK'
    );
  } else {
    console.error(
      '❌ Error actualizando README:',
      put.status,
