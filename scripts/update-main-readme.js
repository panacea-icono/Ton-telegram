...
class MainReadmeUpdater {
  ...
  replaceReposSection(mainReadme, reposSection) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (
        lines[i].includes('## 🎯 Repositorio Principal') ||
    ...
    if (startIndex === -1) {
      ...
      const licenseIndex = lines.findIndex((line) =>
        line.includes('## 📄 Licencia')
      );
      if (licenseIndex !== -1) {
  ...
  run() {
    console.log(
      '🔄 Actualizando README principal con información de repositorios...'
    );
