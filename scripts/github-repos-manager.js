    ...
    try {
      ...
      if (this.scope !== 'user' && this.organization) {
        ...
        if (Array.isArray(orgResp)) {
          ...
          console.log(
            `ℹ️  No se pudo listar org (${status || 'sin código'}). Probando usuario...`
          );
        }
      }
      ...
      if (this.scope === 'user' && this.username) {
        console.log(
          `🔍 Obteniendo repositorios del usuario ${this.username}...`
        );
        const userResp = await this.makeRequest(
          `/users/${this.username}/repos?per_page=100&sort=updated&direction=desc`
        );
      ...
      if (!userListed && this.token) {
        ...
        const selfResp = await this.makeRequest(
      ...
      if (combined.length === 0) {
        throw new Error(
          'No se pudieron obtener repositorios (org y usuario fallaron)'
        );
      }
...
${this.repos
  ...
  .map(
    (repo) =>
      `- [${repo.name}](${repo.url}) - ${new Date(repo.updatedAt).toLocaleDateString('es-ES')}`
  )
  .join('\n')}
...
`;
   ...
   */
  generateCloneScript() {
    const cloneCommands = this.repos
      .map(
        (repo) =>
