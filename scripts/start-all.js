...
function startProcess(service, servicePath, resolve, reject) {
    ...
    child.stdout.on('data', (data) => {
        ...
        output += message;
        process.stdout.write(`${colors.cyan}[${service.name}]${colors.reset} ${message}`);
    });
    
    child.stderr.on('data', (data) => {
        const message = data.toString();
        errorOutput += message;
        process.stderr.write(`${colors.yellow}[${service.name}]${colors.reset} ${message}`);
    });
    ...
    child.on('close', (code) => {
        if (code === 0) {
            ...
            resolve({ success: false, reason: `Process exited with code ${code}`, output, errorOutput });
        }
    });
...
function showServicesStatus(services) {
    ...
    console.log('┌─────────────────────────┬──────────┬──────────┬─────────────┐');
    console.log('│ Servicio                │ Estado   │ Puerto   │ URL         │');
    console.log('├─────────────────────────┼──────────┼──────────┼─────────────┤');
    ...
    services.forEach(service => {
        ...
        console.log(`│ ${service.name.padEnd(23)} │ ${status.padEnd(8)} │ ${port.toString().padEnd(8)} │ ${url.padEnd(11)} │`);
    });
