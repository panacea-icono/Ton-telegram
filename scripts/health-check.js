...
function showBanner() {
    ...
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║                    🌐 PANAS TOKEN ECOSYSTEM                 ║');
...
function showResults(serviceResults, networkResults) {
    ...
    console.log('┌─────────────────────────┬──────────┬──────────┬─────────────┐');
    console.log('│ Servicio                │ Estado   │ Código   │ Tiempo (ms) │');
    console.log('├─────────────────────────┼──────────┼──────────┼─────────────┤');
    ...
    serviceResults.forEach(result => {
        ...
        console.log(`│ ${result.name.padEnd(23)} │ ${status.padEnd(8)} │ ${code.toString().padEnd(8)} │ ${time.toString().padEnd(11)} │`);
        
        if (result.error) {
            console.log(`│ └─ Error: ${result.error.padEnd(20)} │          │          │             │`);
        }
    });
    ...
    networkResults.forEach(result => {
        ...
        console.log(`│ ${url.padEnd(39)} │ ${status.padEnd(8)} │ ${code.toString().padEnd(8)} │`);
    });
...
function showSummary(serviceResults, networkResults) {
    ...
    const overallHealth = (healthyServices + healthyNetworks) / (totalServices + totalNetworks);
