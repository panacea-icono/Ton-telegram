...
function ensureRepositoriesJson() {
  ...
  try {
    ...
    const res = spawnSync('node', ['scripts/github-repos-manager.js', 'json'], {
      encoding: 'utf8',
    });
    if (res.status === 0) {
...
function main() {
  ...
  const recent = [...repos]
    ...
    .map(
      (r) =>
        `- ${r.fullName || r.name} | ${r.language || 'n/a'} | ⭐ ${r.stars} | ${r.isPrivate ? 'private' : 'public'} | ${new Date(r.updatedAt).toISOString()}`
    )
    .join('\n');
  ...
  const file = path.join(
    dir,
    `github-repos-audit-${new Date().toISOString().replace(/[:]/g, '')}.txt`
  );
