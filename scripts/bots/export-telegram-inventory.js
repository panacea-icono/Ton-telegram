...
function main() {
  ...
  if (!fs.existsSync(localPath)) {
    console.error('❌ .env.telegram.local no encontrado (no se commitea). Crea este archivo con tus tokens.');
    process.exit(2);
  }
  ...
  for (const it of items) {
    md.push(`| ${it.env} | ${it.bot_id} | ${it.handle_hint} | ${it.token_masked} |`);
  }
  const dir = path.resolve('audits');
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const file = path.join(dir, `telegram-bots-inventory-${new Date().toISOString().replace(/[:]/g,'')}.md`);
  fs.writeFileSync(file, md.join('\n'));
  fs.writeFileSync(path.join(dir, 'telegram-bots-inventory-latest.md'), md.join('\n'));
