...
class SecureEncryption {
  ...
  encrypt(text, keyInput) {
    ...
    if (key.length !== this.keyLength) {
      throw new Error(
        `Clave inválida: se requieren ${this.keyLength} bytes (hex de 64 chars)`
      );
    }
  ...
  createEncryptedConfig(walletData, outputPath, key) {
    ...
    for (const [walletId, wallet] of Object.entries(walletData)) {
      encryptedData.wallets[walletId] = {
        ...
        accessToken: wallet.accessToken
          ? this.encrypt(wallet.accessToken, key)
          : null,
      ...
      if (wallet.mnemonic) {
        encryptedData.wallets[walletId].encryptedMnemonic =
          this.encryptMnemonic(wallet.mnemonic, key);
      }

      // Encriptar claves privadas si existen
      if (wallet.privateKey) {
        encryptedData.wallets[walletId].encryptedPrivateKey =
          this.encryptPrivateKey(wallet.privateKey, key);
      }
    }
    ...
    console.log(
      `⚠️  IMPORTANTE: Mantener la clave de encriptación en lugar seguro`
    );
  ...
  validateSecurity(config) {
    ...
    for (const [walletId, wallet] of Object.entries(config.wallets)) {
      ...
      if (wallet.privateKey && !wallet.encryptedPrivateKey) {
        issues.push(
          `Wallet ${walletId}: Clave privada en texto plano detectada`
        );
      }
      ...
      if (wallet.securityLevel !== 'HIGH') {
        issues.push(
          `Wallet ${walletId}: Nivel de seguridad bajo (${wallet.securityLevel})`
        );
      }
    }
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    case 'validate':
      ...
      if (!configPath) {
        console.error(
          '❌ Error: Especifica la ruta del archivo de configuración'
        );
        process.exit(1);
      }
      ...
      try {
        ...
        console.log(
          `Estado: ${validation.isValid ? '✅ Seguro' : '❌ Inseguro'}`
        );
